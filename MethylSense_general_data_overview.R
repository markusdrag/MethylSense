...
option_list <- list(
  ...
  make_option(c("--plot_format"),
    ...
    help = "Output plot format(s): png, jpg, svg, pdf, or comma-separated list (e.g., 'png,svg') [default: png]",
    metavar = "character"
  ),
...
save_plot <- function(plot_obj, filename, width = opt$plot_width, height = opt$plot_height) {
  for (format in plot_formats) {
    ...
    if (format == "png") {
      ggsave(filepath, plot_obj, width = width, height = height, dpi = opt$plot_dpi, units = "in")
    } else if (format == "jpg" || format == "jpeg") {
      ggsave(filepath, plot_obj, width = width, height = height, dpi = opt$plot_dpi, units = "in", device = "jpeg")
    } else if (format == "svg") {
      ggsave(filepath, plot_obj, width = width, height = height, units = "in", device = "svg")
    } else if (format == "pdf") {
      ggsave(filepath, plot_obj, width = width, height = height, units = "in", device = "pdf")
    }
...
p1 <- ggplot(dmr_counts_long, aes(x = Region_Size_Label, y = Count, fill = Category)) +
  ...
  scale_y_continuous(labels = comma) + # Add comma formatting
  labs(
...
# Intelligent infection status coloring: Control = dark green, Infected = red
...
for (status in unique_infections) {
  if (grepl("Control|control|CONTROL", status, ignore.case = TRUE)) {
    infection_colors[status] <- "#006400" # Dark green
  } else if (grepl("Infect|infect|INFECT|Asper|asper", status, ignore.case = TRUE)) {
    infection_colors[status] <- "#E31A1C" # Red
  } else {
    infection_colors[status] <- "#FF7F00" # Orange for other statuses
  }
}
...
for (region_size in names(all_meth_objects)) {
  ...
  if (nrow(sig_dmrs) > 0) {
    # Calculate methylation percentages for significant DMRs
    sig_dmr_indices <- seq_len(min(nrow(sig_dmrs), nrow(meth_obj)))
    meth_data <- getData(meth_obj)

    sample_meth_data <- data.frame(Sample_ID = seq_along(meth_obj@sample.ids))
    ...
    for (i in seq_along(meth_obj@sample.ids)) {
      ...
      if (numCs_col %in% colnames(meth_data) && numTs_col %in% colnames(meth_data)) {
        meth_percent <- (meth_data[sig_dmr_indices, numCs_col] /
          (meth_data[sig_dmr_indices, numCs_col] +
            meth_data[sig_dmr_indices, numTs_col])) * 100
        sample_meth_data$Mean_Methylation[i] <- mean(meth_percent, na.rm = TRUE)
      } else {
        sample_meth_data$Mean_Methylation[i] <- NA
      }
    }

    # Add infection status and region size
    if (nrow(sample_metadata) == nrow(sample_meth_data)) {
      sample_meth_data$Infection_Status <- sample_metadata[[opt$infection_col]]
      sample_meth_data$Region_Size_Label <- sprintf("%d KB", as.numeric(region_size) / 1000)
      sample_meth_data$N_DMRs <- nrow(sig_dmrs)

      # Remove NAs
      sample_meth_data <- sample_meth_data[!is.na(sample_meth_data$Infection_Status) &
        !is.na(sample_meth_data$Mean_Methylation), ]

      if (nrow(sample_meth_data) > 0) {
        all_infection_data[[region_size]] <- sample_meth_data
      }
    }
  }
}
...
if (length(all_infection_data) > 0) {
  ...
  # Order infection status (Control first, then Infected)
  combined_infection_data$Infection_Status <- factor(combined_infection_data$Infection_Status,
    levels = sort(unique(combined_infection_data$Infection_Status), decreasing = TRUE)
  )
  ...
  p7 <- ggplot(combined_infection_data, aes(x = Mean_Methylation, y = Infection_Status, fill = Infection_Status)) +
    ...
    labs(
      ...
      subtitle = "Mean methylation across significant DMRs for each region size",
      x = "Mean Methylation (%)",
      y = "Infection Status",
      fill = "Group"
    ) +
    theme_ridges(grid = TRUE, font_size = 12) +
    theme(
      legend.position = "top",
      plot.title = element_text(face = "bold", size = 16),
      strip.background = element_rect(fill = "gray90"),
