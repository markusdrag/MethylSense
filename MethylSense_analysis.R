...
#   FIXED: Replaced METHYLSENSE_COLORS references with standard R color palette in ROC plotting
#   RESOLVED: "argument 'observed' is missing" error traced to theme_methylsense() function
#
# v5.6.3 (2026-01-30) - CPG PLOT GTF ANNOTATION FIX
#   FIXED: CpG methylation plots with GTF annotations now generate successfully
#   FIXED: Replaced theme_methylsense() with explicit theme_minimal() in generate_cpg_barplot()
#   FIXED: Restored original dplyr::coalesce() structure in create_gene_track_plot()
#   FIXED: Forced evaluation of group_names_ordered in plot_roc_curves() to prevent lazy evaluation issues
#
# v5.6.2 (2026-01-29) - CPG PLOT ERROR HANDLING & MANHATTAN PLOT IMPROVEMENT
#   FIXED: Added per-DMR error handling in CpG methylation plot generation loop
#   IMPROVED: Individual DMR plot failures now log detailed error information and continue processing remaining DMRs
#   IMPROVED: Enhanced error tracking with call stack and error class reporting
#   CHANGED: CpG Manhattan plot now uses absolute methylation difference instead of p-values
#   CHANGED: Reference lines at 10% and 20% methylation difference (instead of p-value thresholds)
#
# v5.6.1 (2026-01-28) - ERROR REPORTING & OUTPUT FIXES
#   FIXED: Improved error handling in generate_cpg_report() with conditionMessage()
#   FIXED: Enhanced error reporting with full error details and console flushing
#   FIXED: Suppressed "Picking joint bandwidth" messages from ggridges plots
#   FIXED: Output directory auto-naming - removed debug flag that prevented parameter/timestamp suffix
#   IMPROVED: Better error diagnostics for CpG report generation failures
#   IMPROVED: Added output directory validation and detailed logging
#
# v5.4.1 (2026-01-04) - PLOT IMPROVEMENTS (PUBLIC RELEASE)
#   ADDED: Mean [95% CI] annotations to nested CV fold performance plots
#   ADDED: White background (bg='white') for all plot outputs (PNG, JPEG, SVG)
#   FIXED: Multi-group CpG report generation (2+ groups now supported dynamically)
#   UPDATED: Example GTF file with genes overlapping actual DMR positions
#
# v5.2.0 (2025-12-27) - CPG-LEVEL REPORT FEATURE (PUBLIC RELEASE)
#   ADDED: New flag --cpg_report to generate per-CpG statistics for DMRs
#   ADDED: TSV output with methylation, effect size, Cohen's d, p-value, AUC per CpG
#   ADDED: Manhattan-style plot showing CpG significance genome-wide
#   ADDED: Lollipop plots for individual DMR visualization (top N, default 20)
#   ADDED: Inter-CpG distance calculation for PCR primer design
#   NOTE: Memory intensive - recommended for HPC clusters only
#
...
# Enhanced logging function that writes to both console and log file
# Debug mode (--debug) shows ALL messages; default mode shows only essential output
log_msg <- function(msg, level = "INFO", write_immediately = TRUE, force_console = FALSE) {
  ...
  # Check debug and quiet modes
  is_debug <- exists("opt") && !is.null(opt$debug) && isTRUE(opt$debug)
  is_quiet <- exists("opt") && !is.null(opt$quiet) && isTRUE(opt$quiet)

  # Patterns that indicate essential messages (always shown)
  essential_patterns <- c(
    "^\\[STEP", "^STEP", "^\\[\\d+/\\d+\\]", # Step markers
    "^\\[COMPLETE", "^\\[SUCCESS", "^\\[DONE", # Completion markers
    "^\\[ERROR", "^\\[WARN", "^\\[FAIL", # Errors/warnings
    "^\\[START", "Starting", "Initialising", # Start markers
    "^\\[STRATEGY\\]", "STRATEGY", # Strategy activations
    "^Analysis complete", "^Pipeline complete", # Completion messages
    "^Training", "^Evaluating", "^Model", # ML progress
    "^\\[RESULT", "^\\[SAVED", "^Saved", # Results
    "^\\[DMR\\]", "^DMR", "Top.*DMRs", # DMR results
    "^================", "^-{10,}", # Section dividers
    "accuracy", "AUC", "performance" # Key metrics
  )

  # Patterns that indicate verbose/debug messages (hidden unless --debug)
  verbose_patterns <- c(
    "Processing sample", "Processing chunk", "Processing window",
    "^  ", "^   ", "^    ", # Indented detail lines
    "Total CpGs", "CpG observations", "CpG positions",
    "samples\\)", "windows\\)", "regions\\)",
    "Calculating", "Extracting", "Loading", # Intermediate steps
    "chunk", "batch", "iteration",
    "\\[VERBOSE\\]", "\\[DEBUG\\]",
    "Memory", "memory", "RAM",
    "Elapsed", "elapsed", "took",
    "format\\(", "big.mark" # Formatting details
  )

  # Determine if message is essential or verbose
  is_essential <- any(sapply(essential_patterns, function(p) grepl(p, msg, ignore.case = TRUE)))
  is_verbose <- any(sapply(verbose_patterns, function(p) grepl(p, msg, ignore.case = FALSE)))

  # Show message if: debug mode, essential, or not detected as verbose
  should_show <- is_debug || force_console || (is_essential && !is_quiet) || (!is_verbose && !is_quiet)
  ...
  # Always add to log file (even if hidden from console)
...
verbose_msg <- function(msg) {
  ...
  if (exists("opt") && !is.null(opt$verbose) && opt$verbose && !isTRUE(opt$quiet)) {
    ...
    tryCatch(
      {
        if (format == "svg") {
          ...
          ggsave(output_file, plot_obj,
            width = width, height = height,
            device = "jpeg", dpi = dpi, quality = 95, bg = "white"
          )
...
create_output_structure <- function(base_output_dir) {
  # Only core directories that are ALWAYS needed
  directories <- list(
    ...
    # NOTE: unmatched_samples, cv_analysis_plots, clustering_analysis created on-demand
  )
  ...
  # unmatched_samples: only created on-demand when there are unmatched samples
  # (path stored for later use)
  directories$unmatched_samples <- file.path(base_output_dir, "unmatched_samples")

  return(directories)
}

# =============================================================================
# CPG-LEVEL REPORT FUNCTIONS
# =============================================================================
# Extract individual CpG positions within DMRs for PCR primer design
# =============================================================================

#' Extract CpG-level details from DMRs
#'
#' For each DMR, extract individual CpG positions and calculate per-CpG statistics
#' including methylation by group, effect size, and individual AUC.
#'
#' @param meth_list methylRawList object or list of methylRaw objects
#' @param dmr_df Data frame with DMR information (must have chr, start, end columns)
#' @param treatment_vector Numeric vector of treatment labels (0/1)
#' @param group_names Character vector of group names (length 2)
#' @return data.table with CpG-level statistics
extract_cpg_details <- function(meth_list, dmr_df, treatment_vector, group_names = c("Control", "Case")) {
  # Deduplicate DMRs if present (same chr, start, end)
  if (!inherits(dmr_df, "data.frame")) dmr_df <- as.data.frame(dmr_df)
  # Standardize column names if needed
  if ("seqnames" %in% colnames(dmr_df) && !"chr" %in% colnames(dmr_df)) {
    dmr_df$chr <- dmr_df$seqnames
  }

  if (all(c("chr", "start", "end") %in% names(dmr_df))) {
    dup_idx <- duplicated(dmr_df[, c("chr", "start", "end")])
    if (any(dup_idx)) {
      log_msg(paste("[WARN] Removed", sum(dup_idx), "duplicate DMRs from input list"))
      dmr_df <- dmr_df[!dup_idx, ]
    }
  }
  log_msg("================================================================================")
  log_msg("CPG-LEVEL REPORT v1.1 (2025-12-29)")
  log_msg("Extracting individual CpG statistics for DMR regions")
  log_msg("================================================================================")
  log_msg("[!] This is a memory-intensive operation. HPC recommended.")
  log_msg("================================================================================")
  ...
  # Step 1: Extract all CpG data from samples
  log_msg("")
  log_msg("[STEP 1/4] Extracting CpG data from all samples...")

  all_cpg_data <- list()

  # Create treatment-to-group mapping for multi-group support
  unique_treatments <- sort(unique(treatment_vector))
  treatment_to_group <- setNames(group_names[seq_along(unique_treatments)], as.character(unique_treatments))

  for (i in seq_along(meth_list)) {
    if (i %% 20 == 0 || i == length(meth_list)) {
      log_msg(paste("  Processing sample", i, "/", n_samples))
    }
    sample_data <- getData(meth_list[[i]])
    sample_data$meth_pct <- sample_data$numCs / (sample_data$numCs + sample_data$numTs) * 100
    sample_data$sample_id <- i
    # Multi-group support: map treatment value to group name
    sample_data$group <- treatment_to_group[as.character(treatment_vector[i])]
    all_cpg_data[[i]] <- sample_data[, c("chr", "start", "end", "meth_pct", "sample_id", "group")]
  }

  all_cpgs <- data.table::rbindlist(all_cpg_data)
  log_msg(paste("  Total CpG observations:", format(nrow(all_cpgs), big.mark = ",")))

  # Step 2: Filter CpGs to those within DMRs
  log_msg("")
  log_msg("[STEP 2/4] Filtering CpGs to DMR regions...")

  # Create DMR lookup with unique IDs
  dmr_dt <- data.table::as.data.table(dmr_df)

  # Always create unique DMR IDs using chr:start-end (more meaningful than just numbers)
  dmr_dt$dmr_id <- paste0("chr", dmr_dt$chr, ":", dmr_dt$start, "-", dmr_dt$end)

  # For each CpG, check if it falls within any DMR
  data.table::setkey(all_cpgs, chr, start)

  # More memory-efficient: process per DMR
  cpg_in_dmr_list <- list()

  for (d in seq_len(nrow(dmr_dt))) {
    if (d %% 10 == 0 || d == nrow(dmr_dt)) {
      log_msg(paste("  [PROGRESS] DMR", d, "/", nrow(dmr_dt)))
    }

    dmr_chr <- as.character(dmr_dt$chr[d])
    dmr_start <- dmr_dt$start[d]
    dmr_end <- dmr_dt$end[d]
    dmr_id <- dmr_dt$dmr_id[d]

    # Get CpGs in this DMR
    cpgs_in_range <- all_cpgs[chr == dmr_chr & start >= dmr_start & start <= dmr_end]

    if (nrow(cpgs_in_range) > 0) {
      cpgs_in_range$dmr_number <- d # DMR rank/number
      cpgs_in_range$dmr_id <- dmr_id
      cpgs_in_range$dmr_start <- dmr_start
      cpgs_in_range$dmr_end <- dmr_end
      cpg_in_dmr_list[[d]] <- cpgs_in_range
    }
  }

  if (length(cpg_in_dmr_list) == 0) {
    log_msg("[WARN] No CpGs found within DMR regions!")
    return(NULL)
  }

  cpgs_filtered <- data.table::rbindlist(cpg_in_dmr_list)
  log_msg(paste("  CpGs within DMRs:", format(nrow(cpgs_filtered), big.mark = ",")))

  # Free memory
  rm(all_cpgs, all_cpg_data, cpg_in_dmr_list)
  gc()

  # Step 3: Calculate per-CpG statistics
  log_msg("")
  log_msg("[STEP 3/4] Calculating per-CpG statistics...")

  # For multi-group support: calculate stats for each group dynamically
  n_groups <- length(group_names)

  cpg_stats <- cpgs_filtered[,
    {
      # Calculate stats for each group dynamically
      group_stats <- lapply(group_names, function(gn) {
        meth_vals <- meth_pct[group == gn]
        list(
          n = length(meth_vals),
          mean = if (length(meth_vals) > 0) mean(meth_vals, na.rm = TRUE) else NA_real_,
          sd = if (length(meth_vals) > 1) sd(meth_vals, na.rm = TRUE) else NA_real_
        )
      })
      names(group_stats) <- group_names

      # For effect size: use first group as baseline, last group as comparison
      meth_g1 <- meth_pct[group == group_names[1]]
      meth_gN <- meth_pct[group == group_names[n_groups]]
      mean_g1 <- group_stats[[1]]$mean
      mean_gN <- group_stats[[n_groups]]$mean
      sd_g1 <- group_stats[[1]]$sd
      sd_gN <- group_stats[[n_groups]]$sd

      effect_size <- if (!is.na(mean_g1) && !is.na(mean_gN)) mean_gN - mean_g1 else NA_real_

      # Cohen's d (first vs last group)
      cohens_d <- tryCatch(
        {
          if (length(meth_g1) > 1 && length(meth_gN) > 1 && !is.na(sd_g1) && !is.na(sd_gN)) {
            pooled_sd <- sqrt(((length(meth_g1) - 1) * sd_g1^2 + (length(meth_gN) - 1) * sd_gN^2) /
              (length(meth_g1) + length(meth_gN) - 2))
            if (!is.na(pooled_sd) && pooled_sd > 0) effect_size / pooled_sd else NA_real_
          } else {
            NA_real_
          }
        },
      ...
      # ANOVA p-value for multi-group, t-test for 2-group
      pvalue <- tryCatch(
        {
          if (n_groups == 2) {
            if (length(meth_g1) >= 2 && length(meth_gN) >= 2) {
              as.numeric(t.test(meth_gN, meth_g1)$p.value)
            } else {
              NA_real_
            }
          } else {
            # ANOVA for multi-group
            valid_groups <- sapply(group_stats, function(x) x$n >= 2)
            if (sum(valid_groups) >= 2) {
              grp_factor <- factor(group)
              if (length(unique(grp_factor[!is.na(meth_pct)])) >= 2) {
                aov_result <- tryCatch(aov(meth_pct ~ grp_factor), error = function(e) NULL)
                if (!is.null(aov_result)) summary(aov_result)[[1]][["Pr(>F)"]][1] else NA_real_
              } else {
                NA_real_
              }
            } else {
      ...
      # Single-CpG AUC (first vs last group for simplicity)
      auc <- tryCatch(
        {
          if (length(meth_g1) >= 2 && length(meth_gN) >= 2) {
            n1 <- length(meth_gN)
            n0 <- length(meth_g1)
            greater_count <- sum(outer(meth_gN, meth_g1, function(a, b) {
              ifelse(a > b, 1, ifelse(a == b, 0.5, 0))
            }))
            auc_val <- greater_count / (n1 * n0)
            as.numeric(max(auc_val, 1 - auc_val))
          } else {
            NA_real_
          }
        },
      ...
      # Build output dynamically for all groups
      result <- list(
        effect_size = as.numeric(effect_size),
        cohens_d = as.numeric(cohens_d),
        pvalue = as.numeric(pvalue),
        single_cpg_auc = as.numeric(auc)
      )

      # Add per-group stats
      for (gn in group_names) {
        result[[paste0("n_samples_", gn)]] <- as.integer(group_stats[[gn]]$n)
        result[[paste0("meth_mean_", gn)]] <- as.numeric(group_stats[[gn]]$mean)
        result[[paste0("meth_sd_", gn)]] <- as.numeric(group_stats[[gn]]$sd)
      }

      result
    },
    by = .(dmr_number, dmr_id, dmr_start, dmr_end, chr, start)
  ]


  # Step 4: Calculate distance to next CpG
  log_msg("")
  log_msg("[STEP 4/4] Calculating inter-CpG distances...")

  data.table::setorder(cpg_stats, dmr_id, chr, start)
  cpg_stats[, cpg_index := seq_len(.N), by = dmr_id]
  cpg_stats[, dist_to_next := c(diff(start), NA), by = dmr_id]

  end_time <- Sys.time()
  elapsed <- round(as.numeric(end_time - start_time, units = "mins"), 1)
...
#' Generate CpG-level TSV report
generate_cpg_tsv <- function(cpg_stats, output_dir) {
  if (is.null(cpg_stats) || nrow(cpg_stats) == 0) {
    log_msg("[WARN] No CpG data to write")
    return(NULL)
  }

  output_file <- file.path(output_dir, "cpg_level_dmr_report.tsv")
  data.table::fwrite(cpg_stats, output_file, sep = "\t")
  log_msg(paste("[CpG] Saved TSV report:", basename(output_file)))

  return(output_file)
}


#' Generate Manhattan-style plot for CpG data
generate_cpg_manhattan <- function(cpg_stats, output_dir, plot_formats = c("jpeg")) {
  if (is.null(cpg_stats) || nrow(cpg_stats) == 0) {
    log_msg("[WARN] No CpG data for Manhattan plot")
    return(NULL)
  }

  log_msg("[CpG] Generating Manhattan plot...")

  plot_data <- data.table::copy(cpg_stats)
  
  # Use signed effect size (methylation difference) - shows both hyper and hypo
  plot_data[, meth_diff := effect_size]
  plot_data[is.na(meth_diff), meth_diff := 0]
  
  # Add direction for coloring
  plot_data[, direction := ifelse(meth_diff >= 0, "Hypermethylated", "Hypomethylated")]

  chr_order <- gtools::mixedsort(unique(plot_data$chr))
  plot_data$chr <- factor(plot_data$chr, levels = chr_order)

  chr_lengths <- plot_data[, .(chr_len = max(start)), by = chr]
  chr_lengths <- chr_lengths[match(chr_order, chr_lengths$chr)]
  chr_lengths[, cum_len := cumsum(as.numeric(chr_len)) - chr_len]

  plot_data <- merge(plot_data, chr_lengths[, .(chr, cum_len)], by = "chr")
  plot_data[, bp_cum := start + cum_len]

  # Color by direction (hyper = red, hypo = blue)
  direction_colors <- c("Hypermethylated" = "#D73027", "Hypomethylated" = "#4575B4")

  p <- ggplot2::ggplot(plot_data, ggplot2::aes(x = bp_cum, y = meth_diff, color = direction)) +
    ggplot2::geom_point(alpha = 0.7, size = 1.5) +
    ggplot2::scale_color_manual(values = direction_colors, name = "Direction") +
    ggplot2::scale_x_continuous(
      breaks = chr_lengths$cum_len + chr_lengths$chr_len / 2,
      labels = chr_order, expand = c(0.01, 0)
    ) +
    ggplot2::geom_hline(yintercept = 0, color = "black", linetype = "solid", linewidth = 0.5) +
  ...
  for (fmt in plot_formats) {
    output_file <- file.path(output_dir, paste0("cpg_manhattan_plot.", fmt))
    ggplot2::ggsave(output_file, p, width = 14, height = 6, dpi = 300, bg = "white")
  }
...
#' Generate Genome-Wide Manhattan Plot for ALL DMRs
#'
#' Plots -log10(q-value) for all DMRs identified across all strategies.
#'
...
generate_dmr_manhattan <- function(all_dmrs_path, output_dir, plot_formats = c("png")) {
  ...
  if (nrow(dmrs) == 0) {
    log_msg("[WARN] ALL_DMRs.csv is empty")
    return(NULL)
  }

  # Identify p/q value column
  # Prioritize: qvalue > q-value > adj.P.Val > pvalue > p-value
  col_map <- c("qvalue", "q-value", "adj.P.Val", "pvalue", "p-value", "MWU_p_val", "p_val")
  p_col <- NULL
  for (cname in col_map) {
    if (cname %in% colnames(dmrs)) {
      p_col <- cname
      break
    }
  }
  ...
  if (is.null(p_col)) {
    log_msg("[WARN] No p-value/q-value column found in ALL_DMRs.csv")
    return(NULL)
  }

  log_msg(paste0("[Plot] Using column '", p_col, "' for Manhattan plot"))

  # Detect chromosome column name (could be 'chr' or 'scaffold')
  chr_col <- NULL
  if ("chr" %in% colnames(dmrs)) {
    chr_col <- "chr"
  } else if ("scaffold" %in% colnames(dmrs)) {
    chr_col <- "scaffold"
  } else {
    log_msg("[WARN] No chr/scaffold column found in ALL_DMRs.csv")
    return(NULL)
  }

  log_msg(paste0("[Plot] Using chromosome column: '", chr_col, "'"))

  # Prepare plot data
  plot_data <- dmrs[, c(chr_col, "start", p_col)]
  colnames(plot_data) <- c("chr", "start", "p_raw")

  # Clean p-values
  plot_data$p_raw <- as.numeric(as.character(plot_data$p_raw))
  plot_data <- plot_data[!is.na(plot_data$p_raw), ]

  plot_data$neglog10p <- -log10(plot_data$p_raw + 1e-300)
  plot_data$neglog10p[is.infinite(plot_data$neglog10p)] <- 300 # Cap at 300

  # Sort chromosomes
  # Handle "chr" prefix if mixed
  plot_data$chr_num <- gsub("chr", "", plot_data$chr)
  # Try to convert to numeric for sorting
  chr_order <- unique(plot_data$chr)
  tryCatch({
    suppressWarnings(
      chr_order <- chr_order[order(as.numeric(gsub("[^0-9]", "", chr_order)))]
    )
  })
  ...
  # Infer chromosome lengths from max end position per chr
  chr_lengths <- aggregate(start ~ chr, data = plot_data, max)
  colnames(chr_lengths)[2] <- "chr_len"
  chr_lengths <- chr_lengths[match(chr_order, chr_lengths$chr), ]

  # Add buffer to length to separate chromosomes visually
  chr_lengths$chr_len <- chr_lengths$chr_len * 1.05

  chr_lengths$cum_len <- cumsum(as.numeric(chr_lengths$chr_len)) - chr_lengths$chr_len

  plot_data <- merge(plot_data, chr_lengths[, c("chr", "cum_len")], by = "chr")
  plot_data$bp_cum <- plot_data$start + plot_data$cum_len

  # Colors
  chr_colors <- rep(c("#1f77b4", "#aec7e8"), length.out = length(chr_order))
  names(chr_colors) <- chr_order

  # Plot
  p <- ggplot2::ggplot(plot_data, ggplot2::aes(x = bp_cum, y = neglog10p, color = chr)) +
  ...
  for (fmt in plot_formats) {
    output_file <- file.path(output_dir, paste0("all_dmrs_manhattan.", fmt))
    ggplot2::ggsave(output_file, p, width = 14, height = 7, dpi = 300)
  }
...
#' Generate paired bar chart for CpG methylation in a DMR
#' Shows side-by-side bars for each CpG with error bars, plus difference inset
#' Now supports any number of groups (2+)
#' @param cpg_stats Data frame with CpG statistics
#' @param target_dmr_id DMR identifier to plot
#' @param dmr_number Numeric rank of DMR (for labeling)
#' @param output_dir Output directory
#' @param group_names Character vector of group names (any length >= 2)
#' @param group_colors Character vector of group colors (optional)
#' @param plot_formats Vector of output formats
generate_cpg_barplot <- function(cpg_stats, target_dmr_id, dmr_number, output_dir, group_names,
                                 group_colors = NULL, plot_formats = c("jpeg"), gtf_file = NULL,
                                 max_cpgs = 200) {
  # Filter data for this specific DMR - use which() for reliable filtering
  filter_idx <- which(cpg_stats$dmr_id == target_dmr_id)
  dmr_data <- cpg_stats[filter_idx, ]

  if (nrow(dmr_data) == 0) {
    return(NULL)
  }

  # Sort by position first (needed for subsampling)
  dmr_data <- dmr_data[order(dmr_data$start), ]
  original_n_cpgs <- nrow(dmr_data)
  
  # SUBSAMPLE FOR LARGE DMRs: Pick evenly-spaced CpGs to preserve spatial distribution
  if (nrow(dmr_data) > max_cpgs) {
    subsample_idx <- round(seq(1, nrow(dmr_data), length.out = max_cpgs))
    dmr_data <- dmr_data[subsample_idx, ]
    log_msg(paste("[INFO] Subsampled DMR", target_dmr_id, "from", original_n_cpgs, "to", nrow(dmr_data), "CpGs for plotting"))
  }
  
  # Assign sequential indices after potential subsampling
  dmr_data$cpg_num <- seq_len(nrow(dmr_data))
  n_cpgs <- nrow(dmr_data)
  ...
  bar_data_list <- lapply(seq_along(group_names), function(g) {
    gn <- group_names[g]
    mean_col <- paste0("meth_mean_", gn)
    sd_col <- paste0("meth_sd_", gn)

    data.frame(
      cpg_num = dmr_data$cpg_num,
      cpg_label = paste0("CpG", dmr_data$cpg_num),
      group = gn,
      meth = if (mean_col %in% names(dmr_data)) dmr_data[[mean_col]] else rep(NA, n_cpgs),
      sd = if (sd_col %in% names(dmr_data)) dmr_data[[sd_col]] else rep(0, n_cpgs)
    )
  })
  bar_data <- do.call(rbind, bar_data_list)
  bar_data$sd[is.na(bar_data$sd)] <- 0
  bar_data$group <- factor(bar_data$group, levels = group_names)

  # Difference data for bottom panel - compare first vs last group
  mean_col_first <- paste0("meth_mean_", group_names[1])
  mean_col_last <- paste0("meth_mean_", group_names[n_groups])
  diff_values <- dmr_data[[mean_col_last]] - dmr_data[[mean_col_first]]
  diff_data <- data.frame(
    cpg_num = dmr_data$cpg_num,
    cpg_label = paste0("CpG", dmr_data$cpg_num),
    diff = diff_values,
    direction = ifelse(is.na(diff_values), "Neutral",
      ifelse(diff_values > 0, "Hyper", "Hypo")
    )
  )
  diff_data <- diff_data[!is.na(diff_data$diff), ] # Remove NAs

  chr <- unique(dmr_data$chr)[1]
  dmr_start <- unique(dmr_data$dmr_start)[1]
  dmr_end <- unique(dmr_data$dmr_end)[1]
  n_cpgs <- nrow(dmr_data)
  mean_effect <- round(mean(dmr_data$effect_size, na.rm = TRUE), 1)
  mean_auc <- round(mean(dmr_data$single_cpg_auc, na.rm = TRUE), 2)

  # Main line plot (MethylArtist style)
  # Use color for lines/points and fill for ribbons
  p_lines <- ggplot2::ggplot(bar_data, ggplot2::aes(x = cpg_num, y = meth, color = group, fill = group)) +
    # Ribbon for confidence interval (SD)
    ggplot2::geom_ribbon(
      ggplot2::aes(ymin = pmax(0, meth - sd), ymax = pmin(100, meth + sd)),
      alpha = 0.25, color = NA
    ) +
    # Connecting lines between CpGs
    ggplot2::geom_line(linewidth = 1.2) +
    # Points at each CpG
    ggplot2::geom_point(size = 1.5, shape = 21, stroke = 0.6) +
    ggplot2::scale_color_manual(values = setNames(group_colors, group_names)) +
    ggplot2::scale_fill_manual(values = setNames(group_colors, group_names)) +
  ...
  # Difference plot (inset)
  p_diff <- ggplot2::ggplot(diff_data, ggplot2::aes(x = cpg_num, y = diff, fill = direction)) +
  ...
  # Combine plots - optionally add gene annotation panel
  if (!is.null(gtf_file) && file.exists(gtf_file)) {
    # Parse GTF for this region
    gtf_data <- parse_gtf_region(gtf_file, chr, dmr_start, dmr_end)
    p_genes <- create_gene_track_plot(gtf_data, dmr_start, dmr_end, n_cpgs, gtf_file)
    combined <- patchwork::wrap_plots(p_lines, p_diff, p_genes, ncol = 1, heights = c(3, 1, 0.8))
    plot_height <- 7.5
  } else {
    combined <- patchwork::wrap_plots(p_lines, p_diff, ncol = 1, heights = c(3, 1))
    plot_height <- 6
  }

  # Save
  safe_dmr_id <- gsub(":", "_", target_dmr_id)
  filename_base <- sprintf("DMR_%03d_%s_cpg_profile", dmr_number, safe_dmr_id)

  for (fmt in plot_formats) {
    output_file <- file.path(output_dir, paste0(filename_base, ".", fmt))
    plot_width <- min(20, max(8, n_cpgs * 0.4 + 2)) # Cap at 20 inches
    ggplot2::ggsave(output_file, combined, width = plot_width, height = plot_height, dpi = 300, bg = "white")
  }
...
generate_cpg_lollipop <- generate_cpg_barplot


#' Export DMRs as UCSC browser-compatible BED file
export_ucsc_bed <- function(dmr_df, output_file,
                            track_name = "MethylSense_DMRs",
                            track_description = "Differentially Methylated Regions") {
  log_msg("[BED] Exporting DMRs to UCSC browser format...")
  log_msg(paste("  Output:", output_file))

  # Handle effect_size column - may be called meth.diff or effect_size
  if ("effect_size" %in% names(dmr_df)) {
    effect_vals <- as.numeric(dmr_df$effect_size)
  } else if ("meth.diff" %in% names(dmr_df)) {
    effect_vals <- as.numeric(dmr_df$meth.diff)
  } else {
    effect_vals <- rep(0, nrow(dmr_df))
    log_msg("  [WARN] No effect_size column found, using 0")
  }
  effect_vals[is.na(effect_vals)] <- 0 # Replace NAs

  bed_data <- data.frame(
    chrom = paste0("chr", gsub("^chr", "", dmr_df$chr)),
    chromStart = as.integer(dmr_df$start - 1),
    chromEnd = as.integer(dmr_df$end),
    name = if ("dmr_id" %in% names(dmr_df)) dmr_df$dmr_id else paste0("DMR_", seq_len(nrow(dmr_df))),
    score = pmin(1000, pmax(0, round(abs(effect_vals) * 10))),
    strand = ".",
    thickStart = as.integer(dmr_df$start - 1),
    thickEnd = as.integer(dmr_df$end),
    itemRgb = ifelse(effect_vals > 0, "255,0,0", "0,0,255")
  )
  ...
  writeLines(track_header, output_file)
  write.table(bed_data, output_file,
    sep = "\t", quote = FALSE,
    row.names = FALSE, col.names = FALSE, append = TRUE
  )

  log_msg(paste("  Exported", nrow(bed_data), "DMRs to BED format"))
  return(output_file)
}


#' Parse GTF file and extract features overlapping a region
parse_gtf_region <- function(gtf_file, chr, start, end) {
  if (!file.exists(gtf_file)) {
    warning(paste("GTF file not found:", gtf_file))
    return(NULL)
  }
  ...
  gtf <- tryCatch(
    {
      ...
      gtf_data <- data.table::fread(
        text = gtf_lines, sep = "\t", header = FALSE,
        col.names = c(
          "chr", "source", "feature", "start", "end",
          "score", "strand", "frame", "attribute"
        )
      )
      # Keep ALL features from GTF (removed filtering to gene/exon/CDS/transcript only)
      # This allows regulatory elements, EMARs, etc. to be displayed
      gtf_data
    },
    error = function(e) {
      warning(paste("Error reading GTF:", e$message))
      return(NULL)
    }
  )
  ...
  # Extract gene names from attributes (supports both GTF and GFF3 formats)
  gtf_overlap$gene_name <- sapply(gtf_overlap$attribute, function(attr) {
    # Try GTF format: gene_name "value"
    match <- regmatches(attr, regexpr('gene_name "([^"]+)"', attr))
    if (length(match) > 0) {
      return(gsub('gene_name "|"', "", match))
    }
    # Try GFF3 format: Name=value
    match2 <- regmatches(attr, regexpr("Name=([^;]+)", attr))
    if (length(match2) > 0) {
      return(gsub("Name=", "", match2))
    }
    # Try gene_id as fallback
    match3 <- regmatches(attr, regexpr('gene_id "([^"]+)"', attr))
    if (length(match3) > 0) {
      return(gsub('gene_id "|"', "", match3))
    }
    # Try ID= for GFF3
    match4 <- regmatches(attr, regexpr("ID=([^;]+)", attr))
    if (length(match4) > 0) {
      return(gsub("ID=", "", match4))
    }
    return("Unknown")
  })

  return(as.data.frame(gtf_overlap))
}


#' Create gene annotation track plot for a DMR region (Genome Browser Style)
#'
#' Draws a genome browser-like visualization with:
#' - Thin lines for gene body/introns
#' - Thick rectangles for exons
#' - Strand direction arrows
#' - Overlapping features stacked on separate Y rows
#' - Gene names overlaid on structure
#'
#' @param gtf_data Data frame from parse_gtf_region
#' @param dmr_start Start coordinate of DMR region
#' @param dmr_end End coordinate of DMR region
#' @param n_cpgs Number of CpGs (for x-axis scaling)
#' @param gtf_file Path to GTF file (for display)
create_gene_track_plot <- function(gtf_data, dmr_start, dmr_end, n_cpgs, gtf_file = NULL) {
  ...
  if (!"gene_name" %in% colnames(gtf_data)) gtf_data$gene_name <- NA
  if (!"strand" %in% colnames(gtf_data)) gtf_data$strand <- "."

  # Extract gene names from attributes if not already parsed
  gtf_data$display_label <- dplyr::coalesce(
    gtf_data$gene_name,
    sapply(gtf_data$attribute, function(attr) {
      match <- regmatches(attr, regexpr('gene_name "([^"]+)"', attr))
      if (length(match) > 0) {
        return(gsub('gene_name "|"', "", match))
      }
      match2 <- regmatches(attr, regexpr('gene_id "([^"]+)"', attr))
      if (length(match2) > 0) {
        return(gsub('gene_id "|"', "", match2))
      }
      return("Unknown")
    })
  )

  # Identify unique genes/transcripts for the gene body lines
  genes <- gtf_data[gtf_data$feature %in% c("gene", "transcript"), ]

  # Identify exons and CDS for thick boxes
  exons <- gtf_data[gtf_data$feature == "exon", ]
  cds <- gtf_data[gtf_data$feature == "CDS", ]

  # Identify regulatory elements
  regulatory <- gtf_data[gtf_data$feature %in% c("CpG_island", "enhancer", "promoter"), ]

  # Fall back: if no gene/transcript features, group exons by gene_name
  if (nrow(genes) == 0 && nrow(exons) > 0) {
    gene_groups <- unique(exons$display_label)
    genes <- do.call(rbind, lapply(gene_groups, function(gn) {
      g_exons <- exons[exons$display_label == gn, ]
      data.frame(
        start = min(as.numeric(g_exons$start)),
        end = max(as.numeric(g_exons$end)),
        strand = g_exons$strand[1],
        display_label = gn,
        feature = "gene",
  ...
  # Use a greedy interval scheduling algorithm
  assign_y_levels <- function(features) {
    if (nrow(features) == 0) {
      return(features)
    }
    features$xmin <- scale_pos(as.numeric(features$start))
    features$xmax <- scale_pos(as.numeric(features$end))
    features$xmin <- pmax(0.5, features$xmin)
    features$xmax <- pmin(n_cpgs + 0.5, features$xmax)

    # Sort by start position
    features <- features[order(features$xmin), ]

    # Greedy Y-level assignment
    y_ends <- numeric(0) # Tracks the end position of each Y level
    features$y_level <- NA

    for (i in seq_len(nrow(features))) {
      placed <- FALSE
      for (y in seq_along(y_ends)) {
        if (features$xmin[i] > y_ends[y] + 0.1) { # Small gap for visual separation
          features$y_level[i] <- y
          y_ends[y] <- features$xmax[i]
          placed <- TRUE
          break
        }
      }
      if (!placed) {
        y_ends <- c(y_ends, features$xmax[i])
        features$y_level[i] <- length(y_ends)
      }
    }
    return(features)
  }

  # Process genes with Y-level assignment
  if (nrow(genes) > 0) {
    genes <- assign_y_levels(genes)
    genes$y <- genes$y_level
  }

  # Process regulatory elements with their own Y-levels (below genes)
  if (nrow(regulatory) > 0) {
    regulatory$xmin <- scale_pos(as.numeric(regulatory$start))
    regulatory$xmax <- scale_pos(as.numeric(regulatory$end))
    regulatory$xmin <- pmax(0.5, regulatory$xmin)
    regulatory$xmax <- pmin(n_cpgs + 0.5, regulatory$xmax)
    max_gene_y <- if (nrow(genes) > 0) max(genes$y, na.rm = TRUE) else 0
    regulatory$y <- max_gene_y + 1 # Place regulatory below genes
  }

  # Map exon Y-levels from their parent genes
  if (nrow(exons) > 0 && nrow(genes) > 0) {
    exon_y_map <- setNames(genes$y, genes$display_label)
    exons$y <- exon_y_map[exons$display_label]
    exons$xmin <- scale_pos(as.numeric(exons$start))
    exons$xmax <- scale_pos(as.numeric(exons$end))
    exons$xmin <- pmax(0.5, exons$xmin)
    exons$xmax <- pmin(n_cpgs + 0.5, exons$xmax)
    exons <- exons[!is.na(exons$y), ]
  }

  # Map CDS Y-levels from their parent genes
  if (nrow(cds) > 0 && nrow(genes) > 0) {
    cds_y_map <- setNames(genes$y, genes$display_label)
    cds$y <- cds_y_map[cds$display_label]
    cds$xmin <- scale_pos(as.numeric(cds$start))
    cds$xmax <- scale_pos(as.numeric(cds$end))
    cds$xmin <- pmax(0.5, cds$xmin)
    cds$xmax <- pmin(n_cpgs + 0.5, cds$xmax)
    cds <- cds[!is.na(cds$y), ]
  }

  # Calculate plot Y range
  max_y <- 1
  if (nrow(genes) > 0) max_y <- max(max_y, max(genes$y, na.rm = TRUE))
  if (nrow(regulatory) > 0) max_y <- max(max_y, max(regulatory$y, na.rm = TRUE))

  # Color palette for different genes
  gene_colors <- c("#1E88E5", "#D81B60", "#FFC107", "#004D40", "#8E24AA", "#FF6D00")
  if (nrow(genes) > 0) {
    unique_genes <- unique(genes$display_label)
    gene_color_map <- setNames(
      gene_colors[((seq_along(unique_genes) - 1) %% length(gene_colors)) + 1],
      unique_genes
    )
  }
  ...
  p <- ggplot2::ggplot() +
    ...
    ggplot2::theme(
      ...
      plot.subtitle = ggplot2::element_text(size = 7, color = "gray50", hjust = 0),
      panel.background = ggplot2::element_rect(fill = "white", color = NA),
      plot.background = ggplot2::element_rect(fill = "white", color = NA)
    )

  # Draw gene body lines (thin line for introns)
  if (nrow(genes) > 0) {
    for (i in seq_len(nrow(genes))) {
      g <- genes[i, ]
      gene_color <- gene_color_map[g$display_label]

      # Gene body line (thin)
      p <- p + ggplot2::annotate(
        "segment",
        x = g$xmin, xend = g$xmax,
        y = g$y, yend = g$y,
        color = gene_color, linewidth = 0.8
      )

      # Strand direction arrows along the intron line
      strand_char <- if (is.na(g$strand) || g$strand == ".") "+" else g$strand
      arrow_spacing <- (g$xmax - g$xmin) / 6
      if (arrow_spacing > 0.3) {
        arrow_positions <- seq(g$xmin + arrow_spacing, g$xmax - arrow_spacing, by = arrow_spacing)
        for (arrow_x in arrow_positions) {
          if (strand_char == "+") {
            p <- p + ggplot2::annotate(
              "text",
              x = arrow_x, y = g$y, label = "\u25B6",
              color = gene_color, size = 2, vjust = 0.5
            )
          } else {
            p <- p + ggplot2::annotate(
              "text",
  ...
  if (nrow(exons) > 0) {
    for (i in seq_len(nrow(exons))) {
      e <- exons[i, ]
      exon_color <- gene_color_map[e$display_label]
      p <- p + ggplot2::annotate(
        "rect",
        xmin = e$xmin, xmax = e$xmax,
        ymin = e$y - 0.15, ymax = e$y + 0.15,
        fill = exon_color, color = NA, alpha = 0.9
      )
    }
  }

  # Draw CDS (thicker/darker rectangles on top of exons)
  if (nrow(cds) > 0) {
    for (i in seq_len(nrow(cds))) {
      c <- cds[i, ]
      cds_color <- gene_color_map[c$display_label]
      p <- p + ggplot2::annotate(
        "rect",
        xmin = c$xmin, xmax = c$xmax,
        ymin = c$y - 0.2, ymax = c$y + 0.2,
        fill = cds_color, color = "black", linewidth = 0.3, alpha = 1
      )
    }
  }

  # Draw regulatory elements (different style)
  if (nrow(regulatory) > 0) {
    reg_colors <- c("CpG_island" = "#43A047", "enhancer" = "#FFA000", "promoter" = "#E53935")
    for (i in seq_len(nrow(regulatory))) {
      r <- regulatory[i, ]
      reg_color <- reg_colors[r$feature]
      if (is.na(reg_color)) reg_color <- "#9E9E9E"

      p <- p + ggplot2::annotate(
        "rect",
        xmin = r$xmin, xmax = r$xmax,
        ymin = r$y - 0.12, ymax = r$y + 0.12,
        fill = reg_color, color = NA, alpha = 0.7
      )

      # Label for regulatory element
      p <- p + ggplot2::annotate(
        "text",
        x = (r$xmin + r$xmax) / 2, y = r$y,
        label = r$display_label,
        size = 2.2, fontface = "italic", color = "white"
      )
    }
  }

  # Draw gene name labels LAST so they appear on top of all geometry
  if (nrow(genes) > 0) {
    for (i in seq_len(nrow(genes))) {
      g <- genes[i, ]
      label_x <- (g$xmin + g$xmax) / 2
      p <- p + ggplot2::annotate(
        "label",
        x = label_x, y = g$y + 0.35,
...
#' Generate complete CpG-level report
generate_cpg_report <- function(meth_list, dmr_df, treatment_vector, output_dir,
                                group_names = c("Control", "Case"),
                                group_colors = NULL,
                                top_n_dmrs = 20,
                                plot_formats = c("jpeg"),
                                gtf_file = NULL) {
  log_msg("[DEBUG] Inside generate_cpg_report")
  cpg_dir <- file.path(output_dir, "cpg_level_report")
  if (!dir.exists(cpg_dir)) dir.create(cpg_dir, recursive = TRUE)

  dmr_plots_dir <- file.path(cpg_dir, "dmr_cpg_plots")
  if (!dir.exists(dmr_plots_dir)) dir.create(dmr_plots_dir, recursive = TRUE)

  cpg_stats <- extract_cpg_details(meth_list, dmr_df, treatment_vector, group_names)

  if (is.null(cpg_stats)) {
    log_msg("[ERROR] Failed to extract CpG details")
    return(NULL)
  }

  tsv_path <- generate_cpg_tsv(cpg_stats, cpg_dir)
  generate_cpg_manhattan(cpg_stats, cpg_dir, plot_formats)

  # Get unique DMR IDs
  unique_dmr_ids <- unique(cpg_stats$dmr_id)
  log_msg(paste("[CpG] Found", length(unique_dmr_ids), "unique DMRs with CpG data"))
  log_msg(paste("[CpG] Generating methylation plots for top", min(top_n_dmrs, length(unique_dmr_ids)), "DMRs..."))

  dmr_ids <- unique_dmr_ids[1:min(top_n_dmrs, length(unique_dmr_ids))]

  for (i in seq_along(dmr_ids)) {
    if (i %% 5 == 0 || i == length(dmr_ids)) {
      log_msg(paste("[PROGRESS] Generating plot", i, "/", length(dmr_ids)))
    }
    tryCatch(
      {
        generate_cpg_barplot(cpg_stats, dmr_ids[i], i, dmr_plots_dir, group_names, group_colors, plot_formats, gtf_file)
      },
      error = function(e) {
        log_msg(paste("[WARN] Failed to generate plot for DMR", i, "(", dmr_ids[i], "):", conditionMessage(e)))
        log_msg("[INFO] Continuing with remaining DMRs...")
      }
    )
  }
  ...
  return(list(cpg_stats = cpg_stats, tsv_path = tsv_path, cpg_dir = cpg_dir))
}
...
generate_final_analysis_report <- function(summary_csv_path, output_dir) {
  ...
  md <- c(md, "---")
  md <- c(md, "")
  md <- c(md, "*Report generated by MethylSense v5.2.0 (Public Release)*")
  md <- c(md, "")
  ...
  for (line in md) {
    if (grepl("^# ", line)) {
      ...
      # Table rows
      if (!any(grepl("<table>", tail(html, 3)))) {
        html <- c(html, "<table>")
      }
      ...
      if (grepl("</ul>$", tail(html, 1))) {
        # Already closed
      } else if (any(grepl("<table>", tail(html, 20)))) {
        html <- c(html, "</table>")
      }
      html <- c(html, "<hr>")
    } else if (line == "") {
      ...
      if (any(grepl("<table>", tail(html, 20))) && !any(grepl("</table>", tail(html, 1)))) {
        html <- c(html, "</table>")
      }
      if (nchar(line) > 0) {
        # Handle bold text
...
option_list <- list(
  ...
  make_option(c("-q", "--quiet"),
    ...
    help = "Suppress detailed progress messages (less verbose output) [default: %default]"
  ),
  make_option(c("--qs_file"), type = "character", help = "Path to .qs methylRawList object"),
  make_option(c("--sample_sheet"),
    type = "character", default = NULL,
    help = "Path to sample metadata file (Excel or CSV) for covariate adjustment"
  ),

  # Predefined train/test split option
  ...
  make_option(c("--nested_cv_repeats"),
    type = "integer", default = 1,
    help = "Number of times to repeat the entire nested k-fold CV (default: 1). Each repetition uses a different seed for fold partitioning. For publication: use 5."
  ),

  # CPG-LEVEL REPORT OPTIONS
  make_option(c("--cpg_report"),
    action = "store_true", default = FALSE,
    help = "Generate CpG-level report with individual CpG stats per DMR [HPC recommended, memory intensive]"
  ),
  make_option(c("--cpg_top_dmrs"),
    type = "integer", default = 20,
    help = "Number of top DMRs to generate lollipop plots for [default: %default]"
  ),
  make_option(c("--force_cpg"),
    action = "store_true", default = FALSE,
    help = "Force regeneration of CpG report even if already exists"
  ),
  make_option(c("--gtf_file"),
    type = "character", default = NULL,
    help = "GTF annotation file for gene overlay in CpG plots [optional]"
  ),
  make_option(c("--export_bed"),
    action = "store_true", default = TRUE,
    help = "Export DMRs as UCSC browser-compatible BED file [default: TRUE]"
  ),

  # ADD THESE LINES AFTER THE EXISTING make_option() ENTRIES:
  make_option(c("--qvalue_threshold"),
    type = "double", default = 0.05,
    help = "Q-value threshold for DMR significance [default: %default]"
  ),
  ...
  make_option(c("--covariate_cols"),
    type = "character", default = NULL,
    help = "Comma-separated column names from sample sheet to use as DMR covariates (e.g., 'Batch,Storage_Days') [default: none]"
  ),
...
# Parse and validate group colors
...
if (!is.null(opt$group_colors)) {
  ...
  # AUTO-ASSIGN from MethylSense palette if not provided
  group_colors_list <- METHYLSENSE_COLORS$Extended[1:length(group_names_list)]
  for (i in seq_along(group_names_list)) {
    nm <- group_names_list[i]
    if (grepl("Control", nm, ignore.case = TRUE)) {
      group_colors_list[i] <- METHYLSENSE_COLORS$Control
    } else if (grepl("Infect|Asper", nm, ignore.case = TRUE)) {
      group_colors_list[i] <- METHYLSENSE_COLORS$Infected
    } else if (grepl("Suspect", nm, ignore.case = TRUE)) {
      group_colors_list[i] <- METHYLSENSE_COLORS$Suspected
    }
  }
  log_msg(paste("[COLORS] Auto-assigned colors from MethylSense palette:", paste(group_colors_list, collapse = ", ")))
}

# Create named color vector
if (!is.null(group_colors_list) && !is.null(group_names_list)) {
  names(group_colors_list) <- group_names_list
  log_msg(paste("[COLORS] Final group colors:"))
  for (i in seq_along(group_names_list)) {
...
if (is.null(opt$nested_cv_repeats)) opt$nested_cv_repeats <- 1

if (opt$cv_repeats > 0) {
  log_msg(paste("[CV] Cross-validation enabled:", opt$cv_repeats, "repeats"))
  if (opt$nested_cv) {
    log_msg(paste("[CV] Using nested cross-validation (gold standard) |",
                  opt$cv_repeats, "folds x", opt$nested_cv_repeats, "repetitions =",
                  opt$cv_repeats * opt$nested_cv_repeats, "total evaluations"))
  }
} else {
...
apply_predefined_split <- function(input_data, assignments, sample_names, output_dirs) {
  ...
  if (length(unmatched_samples) > 0) {
    ...
    # SAVE UNMATCHED SAMPLES TO ORGANIZED DIRECTORY (create on-demand)
    if (length(unmatched_samples) > 0) {
      dir.create(output_dirs$unmatched_samples, recursive = TRUE, showWarnings = FALSE)
  ...
  # Return data from input_data parameter
  return(list(
...
process_treatment_groups <- function(treatment_info, group_names = NULL, treatment_mapping = NULL) {
  ...
  if (!is.null(treatment_mapping)) {
    ...
    mapping_dict <- list()
    mapped_names_in_order <- character(length(mapping_pairs))
  ...
  # Store group colors if provided
  group_colors <- NULL
...
perform_cross_validation <- function(ml_data_subset, model_name, model_specs,
  ...
  log_msg(paste("N folds/repeats:", n_repeats))
  log_msg(paste("Nested:", nested))
  if (nested) log_msg(paste("Nested CV repetitions:", n_cv_repeats))
  log_msg("====================================")
  flush.console()
  ...
  if (nested) {
    total_evaluations <- n_repeats * n_cv_repeats
    log_msg(paste("[NESTED CV] Starting", n_repeats, "outer folds x", n_cv_repeats,
                  "repetitions =", total_evaluations, "total evaluations for", model_name))
  } else {
  ...
  # Determine loop structure based on CV type
  if (nested) {
    # Nested CV: repeat the entire k-fold procedure n_cv_repeats times
    eval_counter <- 0
    for (rep_idx in 1:n_cv_repeats) {
      # Create non-overlapping stratified folds for this repetition
      set.seed(123 + (rep_idx - 1) * 10)
      nested_folds <- createFolds(ml_data_subset$label, k = n_repeats, list = TRUE)

      if (n_cv_repeats > 1) {
        log_msg(paste("  [NESTED CV] Repetition", rep_idx, "of", n_cv_repeats))
      }

      for (fold_idx in 1:n_repeats) {
        eval_counter <- eval_counter + 1
        verbose_msg(paste("  Outer fold", fold_idx, "of", n_repeats,
                          if (n_cv_repeats > 1) paste0("(rep ", rep_idx, ")") else ""))
          ...
          capture.output(
            {
              tryCatch(
                {
                  # Nested CV: use pre-created non-overlapping stratified folds
                  test_indices <- nested_folds[[fold_idx]]
                ...
                tryCatch(
                  {
                    ...
                    if (pos_class %in% colnames(pred_prob_cv)) {
                      roc_obj <- pROC::roc(
                        ...
                        predictor = pred_prob_cv[, pos_class],
                      ...
                      # Store ROC data for nested CV visualisation
                      fold_roc_data[[eval_counter]] <- data.frame(
    ...
    # Regular CV: Monte Carlo random sub-sampling
    for (fold_idx in 1:n_repeats) {
                  ...
                  tryCatch(
                    {
                      ...
                      if (pos_class %in% colnames(pred_prob_cv)) {
                        ...
                        auc_cv <- as.numeric(pROC::auc(roc_obj))
                        if (is.na(auc_cv) || !is.finite(auc_cv)) auc_cv <- 0
                      }
                    },
                    error = function(e) { auc_cv <- 0 }
                  )
                } else if (n_classes > 2) {
                  auc_values <- numeric(n_classes)
                  for (i in 1:n_classes) {
                    ...
                    if (class_name %in% colnames(pred_prob_cv)) {
                      tryCatch(
                        ...
                        error = function(e) { auc_values[i] <- 0 }
                      )
                    }
                  }
                  auc_cv <- mean(auc_values, na.rm = TRUE)
  ...
  return(list(
    ...
    fold_roc_data = if (nested) fold_roc_data else NULL
  ))
}
  ...
  tryCatch(
    {
      ...
      if (is.null(group_colors)) {
        ...
        group_colors <- METHYLSENSE_COLORS$Extended[1:n_groups]
        for (i in seq_along(group_names_ordered)) {
          nm <- group_names_ordered[i]
          if (grepl("Control", nm, ignore.case = TRUE)) {
            group_colors[i] <- METHYLSENSE_COLORS$Control
          } else if (grepl("Infect|Asper", nm, ignore.case = TRUE)) {
            group_colors[i] <- METHYLSENSE_COLORS$Infected
          } else if (grepl("Suspect", nm, ignore.case = TRUE)) group_colors[i] <- METHYLSENSE_COLORS$Suspected
        }
        names(group_colors) <- group_names_ordered
      }
  ...
  tryCatch(
    {
      ...
      if (is.null(group_colors_to_use)) {
        n_groups <- length(group_names_ordered)
        # Use MethylSense extended palette
        group_colors_to_use <- METHYLSENSE_COLORS$Extended[1:n_groups]
        for (i in seq_along(group_names_ordered)) {
          nm <- group_names_ordered[i]
          if (grepl("Control", nm, ignore.case = TRUE)) {
            group_colors_to_use[i] <- METHYLSENSE_COLORS$Control
          } else if (grepl("Infect|Asper", nm, ignore.case = TRUE)) {
            group_colors_to_use[i] <- METHYLSENSE_COLORS$Infected
          } else if (grepl("Suspect", nm, ignore.case = TRUE)) group_colors_to_use[i] <- METHYLSENSE_COLORS$Suspected
        }
        names(group_colors_to_use) <- group_names_ordered
      }
      ...
      if (!is.null(group_colors)) {
        ...
        # Generate default colors - Use MethylSense extended palette
        n_groups <- length(group_names_ordered)
        group_colors_to_use <- METHYLSENSE_COLORS$Extended[1:n_groups]
        for (i in seq_along(group_names_ordered)) {
          nm <- group_names_ordered[i]
      ...
      if (exists("treatment_processed") && !is.null(treatment_processed) &&
        ...
        # Default colors - Use MethylSense extended palette
        group_colors <- METHYLSENSE_COLORS$Extended[1:n_groups]
        for (i in seq_along(group_names_ordered)) {
          nm <- group_names_ordered[i]
          if (grepl("Control", nm, ignore.case = TRUE)) {
      ...
      # Create individual sample data for violin plot
      ...
      for (group in group_names_ordered) {
        ...
        if (length(group_indices) > 0) {
          ...
          violin_data <- rbind(violin_data, data.frame(
            Group = group,
            Methylation = sample_means,
            stringsAsFactors = FALSE
          ))
        }
      }
      violin_data$Group <- factor(violin_data$Group, levels = group_names_ordered)

      # Create plot with violin + boxplot + points
      p_summary <- ggplot(violin_data, aes(x = Group, y = Methylation, fill = Group)) +
        geom_violin(alpha = 0.6, scale = "width", trim = FALSE) +
        geom_boxplot(width = 0.15, alpha = 0.8, outlier.shape = NA) +
        geom_jitter(aes(color = Group), width = 0.1, alpha = 0.7, size = 2) +
  ...
  tryCatch(
    {
      ...
      # CRITICAL FIX: Force evaluation of group_names_ordered to prevent lazy evaluation issues
      # This must happen immediately after setting group_names_ordered
      group_names_ordered <- as.character(group_names_ordered)
        ...
        tryCatch(
          {
            ...
            # Create plot with defensive settings (suppress bandwidth messages)
            p_prob_ridge_simple <- suppressMessages(ggplot(
              prob_long,
              aes(
                x = probability,
                y = predicted_class,
        ...
        tryCatch(
          {
            ...
            if (has_jitter_position) {
              # NEWER VERSION: Use built-in jittered points (suppress bandwidth messages)
              p_prob_ridge_jitter <- suppressMessages(ggplot(
                prob_long,
                aes(
                  x = probability,
                  y = predicted_class,
              ...
              # OLDER VERSION: Manual overlay with geom_point (suppress bandwidth messages)
...
# ================================================================================
# STANDARD CV VISUALIZATION SUITE
# ================================================================================

plot_standard_cv_results <- function(cv_result_regular,
                                     model_name, output_dir, region_label, n_markers) {
  if (is.null(cv_result_regular)) {
    log_msg("[PLOT] No standard CV results to plot")
    return(NULL)
  }
  ...
  tryCatch(
    {
      ...
      ci_data$ci_lower <- ci_data$mean_val - 1.96 * ci_data$sd_val / sqrt(n_folds)
      ci_data$ci_upper <- ci_data$mean_val + 1.96 * ci_data$sd_val / sqrt(n_folds)
      ci_data$label <- sprintf(
        "Mean: %.2f [%.2f-%.2f]",
        ci_data$mean_val, ci_data$ci_lower, ci_data$ci_upper
      )
      ...
      p_fold_performance <- ggplot(fold_long, aes(x = fold, y = value, color = metric, group = metric)) +
        ...
        # Add Mean [95% CI] annotation in bottom-right corner of each facet
        geom_text(
          data = ci_data,
          aes(x = Inf, y = 0.05, label = label),
          hjust = 1.1, vjust = 0, size = 3.5, color = "black", fontface = "bold",
          inherit.aes = FALSE
        ) +
      ...
      fold_performance_file <- file.path(standard_dir, "standard_cv_fold_performance.png")
  ...
  # ===== PLOT 2: Metric Distributions (Raincloud Plot) =====
  tryCatch(
    {
      fold_data <- cv_result_regular$fold_results
      fold_data$fold <- 1:nrow(fold_data)
      ...
      raincloud_file <- file.path(standard_dir, "standard_cv_distributions.png")
      save_plot_universal(p_raincloud, raincloud_file, width = 10, height = 8)
      plot_files$distributions <- raincloud_file

      log_msg("[PLOT] Standard CV distribution plot created")
    },
    error = function(e) {
      log_error(paste("Standard CV distribution plot failed:", e$message), "Plotting")
    }
  )
...
if (!is.null(opt$train_test_file)) {
  ...
  if (opt$test_ratio != 0.3) { # 0.3 is the default (70:30 split)
...
all_dirs <- list.dirs(base_dir, full.names = TRUE, recursive = FALSE)
  # ... (rest of original logic matches replacement target)
...
# Strategy and config info (suppress verbose lines in quiet mode)
...
# Show group names and colors if specified
if (!is.null(opt$group_names)) {
...
if (opt$verbose && !isTRUE(opt$quiet)) cat("[VERBOSE] VERBOSE MODE ENABLED\n")
...
# ---------------------------
# Load sample metadata for covariate adjustment (optional)
# ---------------------------
sample_metadata <- NULL
if (!is.null(opt$sample_sheet) && nchar(opt$sample_sheet) > 0) {
  log_msg(paste("[DATA] Loading sample metadata from:", basename(opt$sample_sheet)))
  if (grepl("\\.xlsx$", opt$sample_sheet, ignore.case = TRUE)) {
    sample_metadata <- readxl::read_excel(opt$sample_sheet)
  } else {
    sample_metadata <- read.csv(opt$sample_sheet, stringsAsFactors = FALSE)
  }
  sample_metadata <- as.data.frame(sample_metadata)
  log_msg(paste("[OK] Loaded metadata for", nrow(sample_metadata), "samples with", ncol(sample_metadata), "columns"))
  log_msg(paste("[DATA] Available columns:", paste(colnames(sample_metadata), collapse = ", ")))
} else if (!is.null(opt$covariate_cols) && nchar(opt$covariate_cols) > 0) {
  log_msg("[WARN] --covariate_cols specified but --sample_sheet not provided")
  log_msg("[WARN] Covariates will be ignored. Provide --sample_sheet to enable covariate adjustment.")
}
...
if (length(valid_samples) < length(meth)) {
  excluded_count <- length(meth) - length(valid_samples)
  excluded_samples <- names(meth)[sample_sizes < opt$min_cpg]
  log_msg(paste("[QC] Filtering:", excluded_count, "samples with <", format(opt$min_cpg, big.mark = ","), "CpGs"))
  if (length(excluded_samples) <= 10) {
    log_msg(paste("[QC] Excluded samples:", paste(excluded_samples, collapse = ", ")))
  } else {
    log_msg(paste("[QC] Excluded samples (first 10):", paste(head(excluded_samples, 10), collapse = ", "), "..."))
  }

  if (length(valid_samples) == 0) {
    stop(paste("[ERROR] No samples passed min_cpg filter (", opt$min_cpg, "). Lower --min_cpg or check your data."))
  }
...
log_msg(paste("[OK] Using", length(meth), "samples for analysis (min CpG threshold:", format(opt$min_cpg, big.mark = ","), ")"))
...
model_categories <- list(
  ...
  # All models except xgboost (faster execution)
...
if (length(region_data) == 0 && !opt$force_ml) {
  ...
  # Run CpG report even from cached exit (if requested)
  if (isTRUE(opt$cpg_report)) {
    log_msg("")
    log_msg("[CPG REPORT] Running CpG-level report from cached results...")

    cpg_tsv_path <- file.path(opt$output_dir, "cpg_level_report", "cpg_level_dmr_report.tsv")
    if (file.exists(cpg_tsv_path) && !isTRUE(opt$force_cpg)) {
      log_msg(paste("[CPG REPORT] Found existing report:", cpg_tsv_path))
      log_msg("[CPG REPORT] Skipping (use --force_cpg to regenerate)")
    } else {
      all_dmr_files <- list.files(opt$output_dir,
        pattern = "dmr_summary_.*\\.csv$",
        full.names = TRUE, recursive = TRUE
      )
      if (length(all_dmr_files) > 0) {
        all_dmrs <- data.table::rbindlist(lapply(all_dmr_files, function(f) {
          df <- read.csv(f, stringsAsFactors = FALSE)
          df$source_file <- basename(f)
          df
        }), fill = TRUE)

        if ("scaffold" %in% names(all_dmrs) && !"chr" %in% names(all_dmrs)) {
          all_dmrs$chr <- all_dmrs$scaffold
        }
        ...
        if (all(c("chr", "start", "end") %in% names(all_dmrs))) {
          ...
          tryCatch(
            {
              cpg_result <- generate_cpg_report(
                meth_list = meth,
                dmr_df = all_dmrs,
                treatment_vector = as.numeric(meth@treatment),
              ...
              if (!is.null(cpg_result)) {
                log_msg("[CPG REPORT] Successfully generated CpG-level report")
              }

              # Export BED file if requested
              if (isTRUE(opt$export_bed)) {
                bed_file <- file.path(opt$output_dir, "dmr_regions.bed")
                export_ucsc_bed(all_dmrs, bed_file)

                # ===== REGENERATE PER-MODEL BED FILES FROM CACHED DATA =====
                # Find existing model directories with dmr_details_with_coordinates.csv
                log_msg("[BED] Regenerating per-model BED files from cached data...")
                model_detail_files <- list.files(opt$output_dir,
                  pattern = "dmr_details_with_coordinates\\.csv$",
                  full.names = TRUE, recursive = TRUE
                )
                if (length(model_detail_files) > 0) {
                  log_msg(paste("[BED] Found", length(model_detail_files), "model result files"))
                  for (detail_file in model_detail_files) {
                    tryCatch(
                      {
                        detailed_dmr_info <- read.csv(detail_file, stringsAsFactors = FALSE)
                        if (nrow(detailed_dmr_info) > 0 && "scaffold" %in% names(detailed_dmr_info)) {
                          model_dir <- dirname(detail_file)
                          n_markers <- nrow(detailed_dmr_info)
                          model_name <- basename(dirname(model_dir)) # Parent dir is model name

                          # Get positive class from data if available
                          pos_class <- if ("positive_class" %in% names(detailed_dmr_info)) {
                            detailed_dmr_info$positive_class[1]
                          } else {
                            "Unknown"
                          }

                          # Get accuracy if available
                          accuracy <- if ("accuracy" %in% names(detailed_dmr_info)) {
                            round(detailed_dmr_info$accuracy[1], 3)
                          } else {
                            "N/A"
                          }

                          bed_file_model <- file.path(model_dir, paste0("dmr_regions_", n_markers, "markers.bed"))
                          track_name <- paste0("MethylSense_", model_name, "_", n_markers, "markers")
                          track_desc <- paste0(
                            "DMRs for ", model_name, " | Accuracy: ", accuracy,
                            " | Positive: ", pos_class, " | Red=Hyper Blue=Hypo"
                          )

                          # Create BED data
                          bed_data <- data.frame(
                            chrom = paste0("chr", gsub("^chr", "", detailed_dmr_info$scaffold)),
                            chromStart = as.integer(detailed_dmr_info$start - 1),
                            chromEnd = as.integer(detailed_dmr_info$end),
                            name = if ("DMR_ID" %in% names(detailed_dmr_info)) {
                              paste0(
                                detailed_dmr_info$DMR_ID, "_rank", seq_len(nrow(detailed_dmr_info)),
                                "_", ifelse(grepl("Hyper", detailed_dmr_info$direction, ignore.case = TRUE), "HYPER", "HYPO")
                              )
                            } else {
                              paste0("DMR_", seq_len(nrow(detailed_dmr_info)))
                            },
                            score = if ("meth_difference_actual" %in% names(detailed_dmr_info)) {
                              pmin(1000, pmax(0, round(abs(detailed_dmr_info$meth_difference_actual) * 100)))
                            } else {
                              500
                            },
                            strand = ".",
                            thickStart = as.integer(detailed_dmr_info$start - 1),
                            thickEnd = as.integer(detailed_dmr_info$end),
                            itemRgb = ifelse(grepl("Hyper", detailed_dmr_info$direction, ignore.case = TRUE), "255,0,0", "0,0,255"),
                            stringsAsFactors = FALSE
                          )

                          # Write BED file with track header
                          track_header <- paste0(
                            "track name=\"", track_name, "\" description=\"", track_desc,
                            "\" visibility=2 itemRgb=\"On\" useScore=1"
                          )
                          writeLines(track_header, bed_file_model)
                          write.table(bed_data[complete.cases(bed_data), ], bed_file_model,
                            sep = "\t",
                            quote = FALSE, row.names = FALSE, col.names = FALSE, append = TRUE
                          )
                          log_msg(paste0("[BED] Regenerated: ", basename(bed_file_model)))
                        }
                      },
                      error = function(e) {
                        log_msg(paste("[WARN] Could not regenerate BED for:", detail_file, "-", e$message))
                      }
                    )
                  }
                  log_msg("[BED] Per-model BED regeneration complete")
                } else {
                  log_msg("[WARN] No model result files found for BED regeneration")
                }
                # ===== END PER-MODEL BED REGENERATION =====
              }

              # Generate Genome-Wide Manhattan Plot on Rerun
              dmr_files <- list.files(opt$output_dir, pattern = "ALL_DMRs_complete_dataset_.*\\.csv", full.names = TRUE, recursive = TRUE)
              if (length(dmr_files) > 0) {
                log_msg(paste("[Plot] Found DMR file for Manhattan plot:", basename(dmr_files[1])))
                tryCatch(
                  {
                    generate_dmr_manhattan(dmr_files[1], opt$output_dir)
                  },
                  error = function(e) {
                    log_msg(paste("[WARN] Manhattan plot failed:", e$message))
                    log_msg("[WARN] Continuing without Manhattan plot...")
                  }
                )
              } else {
                log_msg("[WARN] No ALL_DMRs.csv found for Manhattan plot")
              }
            },
            error = function(e) {
              error_msg <- if (is.null(e$message) || e$message == "") conditionMessage(e) else e$message
              log_msg("[ERROR] CPG REPORT GENERATION FAILED")
              log_msg(paste("  Error:", error_msg))
              log_msg("[HINT] This may be due to insufficient memory, missing DMR data, or plotting issues")
            }
          )
        }
      } else {
        log_msg("[CPG REPORT] No DMR files found")
      }
    }
  }

  cat(paste0("[COMPLETE] Analysis complete (resumed from cache)! | MethylSense v", SCRIPT_VERSION, "\n"))
  quit(save = "no", status = 0)
}
  ...
  tryCatch(
    {
      ...
      if (!goto_ml_training) {
        ...
        # Prepare covariates if specified
        cov_df <- NULL
        if (!is.null(opt$covariate_cols) && nchar(opt$covariate_cols) > 0) {
          cov_names <- trimws(strsplit(opt$covariate_cols, ",")[[1]])
          log_msg(paste("[COVA] Adjusting DMR calling for covariates:", paste(cov_names, collapse = ", ")))

          # Get sample IDs from united object in same order
          sample_ids <- getSampleID(united)

          # Check if sample_metadata is available (should be loaded earlier)
          if (exists("sample_metadata") && !is.null(sample_metadata)) {
            # Match sample order
            sample_order <- match(sample_ids, sample_metadata$ID)
            if (any(is.na(sample_order))) {
              log_msg("[WARN] Some samples not found in metadata, skipping covariates")
            } else {
              # Check if all covariate columns exist
              missing_cols <- cov_names[!cov_names %in% colnames(sample_metadata)]
              if (length(missing_cols) > 0) {
                log_msg(paste("[WARN] Missing covariate columns:", paste(missing_cols, collapse = ", ")))
                cov_names <- cov_names[cov_names %in% colnames(sample_metadata)]
              }

              if (length(cov_names) > 0) {
                cov_df <- sample_metadata[sample_order, cov_names, drop = FALSE]
                # Convert character columns to factors
                for (col in names(cov_df)) {
                  if (is.character(cov_df[[col]])) {
            ...
            log_msg("[WARN] sample_metadata not found, skipping covariates")
          }
        }
        ...
        # Generate Genome-Wide Manhattan Plot (wrapped in tryCatch to prevent crashes)
        tryCatch(
          {
            generate_dmr_manhattan(all_dmrs_file, output_dirs$complete_dmr_datasets, plot_formats = plot_formats)
          },
          error = function(e) {
            log_msg(paste("[WARN] Manhattan plot generation failed:", e$message))
            log_msg("[WARN] Continuing with ML training despite plot error...")
          }
        )
        ...
        # Save ALL DMRs (not just significant ones) for record keeping
        ...
        if (max_accuracy >= opt$min_accuracy) {
          ...
          for (model_name in names(marker_results)) {
            ...
            if (result$accuracy >= opt$min_accuracy) {
              ...
              detailed_dmr_info <- tryCatch(
                {
                  data.frame(
                    ...
                    # Use calculated actual difference and direction
              ...
              # ===== EXPORT PER-MODEL BED FILE FOR UCSC BROWSER =====
              # Export BED file with DMRs used by this specific model for UCSC visualization
              if (isTRUE(opt$export_bed) && nrow(detailed_dmr_info) > 0) {
                tryCatch(
                  {
                    bed_file <- file.path(model_dir, paste0("dmr_regions_", n_markers, "markers.bed"))

                    # Create informative track name and description
                    track_name <- paste0("MethylSense_", model_name, "_", n_markers, "markers")
                    track_desc <- paste0(
                      "DMRs for ", model_name, " model with ", n_markers, " markers | ",
                      "Accuracy: ", round(result$accuracy, 3), " | ",
                      "Positive class: ", pos_class, " | ",
                      "Red=Hyper Blue=Hypo in ", pos_class
                    )

                    # Prepare BED data with all metadata
                    # BED format: chrom, chromStart (0-based), chromEnd, name, score, strand, thickStart, thickEnd, itemRgb
                    bed_data <- data.frame(
                      chrom = paste0("chr", gsub("^chr", "", detailed_dmr_info$scaffold)),
                      chromStart = as.integer(detailed_dmr_info$start - 1), # Convert to 0-based
                      chromEnd = as.integer(detailed_dmr_info$end),
                      # Name: DMR_ID with rank and direction
                      name = paste0(
                        detailed_dmr_info$DMR_ID,
                        "_rank", detailed_dmr_info$DMR_rank,
                        "_", ifelse(grepl("Hyper", detailed_dmr_info$direction), "HYPER", "HYPO")
                      ),
                      # Score: Scale meth difference (0-1000 range for UCSC)
                      score = pmin(1000, pmax(0, round(abs(detailed_dmr_info$meth_difference_actual) * 100))),
                      strand = ".",
                      thickStart = as.integer(detailed_dmr_info$start - 1),
                      thickEnd = as.integer(detailed_dmr_info$end),
                      # Color: Red for hyper, Blue for hypo (relative to positive class)
                      itemRgb = ifelse(grepl("Hyper", detailed_dmr_info$direction), "255,0,0", "0,0,255"),
                      stringsAsFactors = FALSE
                    )
                    ...
                    if (nrow(bed_data) > 0) {
                      # Write track header with detailed description
                      track_header <- paste0(
                        "track name=\"", track_name, "\" ",
                      ...
                      # Write file
                      writeLines(track_header, bed_file)
                      write.table(bed_data, bed_file,
                        sep = "\t", quote = FALSE,
                        row.names = FALSE, col.names = FALSE, append = TRUE
                      )

                      log_msg(paste0("[BED] Exported ", nrow(bed_data), " DMRs to: ", basename(bed_file)))
                      log_msg(paste0("      Track: ", track_name))
                      log_msg(paste0(
                        "      Hyper (red): ", sum(grepl("Hyper", detailed_dmr_info$direction)),
                        " | Hypo (blue): ", sum(grepl("Hypo", detailed_dmr_info$direction))
                      ))
                    }
                  },
                  error = function(e) {
                    log_msg(paste("[WARN] Per-model BED export failed:", e$message))
                  }
                )
              }
              # ===== END BED EXPORT =====
              ...
              if (opt$cv_repeats > 0) {
                ...
                if (opt$create_plots) {
                  ...
                  if (!is.null(standard_cv_plots)) {
                    log_msg(paste("[OK] Created", length(standard_cv_plots), "standard CV plots"))
                    for (plot_name in names(standard_cv_plots)) {
                      log_msg(paste("    -", plot_name, ":", basename(standard_cv_plots[[plot_name]])))
                    }
                  }
                }
                ...
                if (opt$nested_cv) {
                  ...
                  cv_result_nested <- perform_cross_validation(
                    ...
                    n_markers = n_markers,
                    nested = TRUE,
                    n_cv_repeats = opt$nested_cv_repeats
                  )
              ...
              if (opt$create_plots) {
                ...
                if (exists("treatment_processed") && !is.null(treatment_processed)) {
                  # Match by rownames instead of positional indexing
                ...
                tryCatch(
                  {
                    ...
                    if (is.null(point_colors)) {
                      n_groups <- length(group_names_ordered)
                      # Fallback to MethylSense palette
                      point_colors <- METHYLSENSE_COLORS$Extended[1:n_groups]
                      for (i in seq_along(group_names_ordered)) {
                        nm <- group_names_ordered[i]
                        if (grepl("Control", nm, ignore.case = TRUE)) {
                          point_colors[i] <- METHYLSENSE_COLORS$Control
                        } else if (grepl("Infect|Asper", nm, ignore.case = TRUE)) {
                          point_colors[i] <- METHYLSENSE_COLORS$Infected
                        } else if (grepl("Suspect", nm, ignore.case = TRUE)) point_colors[i] <- METHYLSENSE_COLORS$Suspected
                      }
                      names(point_colors) <- group_names_ordered
                    }
...
# ================================================================================
# CPG-LEVEL REPORT (if requested)
# ================================================================================
if (isTRUE(opt$cpg_report)) {
  log_msg("")
  log_msg("================================================================================")
  log_msg("[CPG REPORT] CpG-level report requested")
  log_msg("================================================================================")

  # Check if already completed (caching) - respect --force_cpg
  cpg_tsv_path <- file.path(opt$output_dir, "cpg_level_report", "cpg_level_dmr_report.tsv")
  if (file.exists(cpg_tsv_path) && !isTRUE(opt$force_cpg)) {
    log_msg(paste("[CPG REPORT] Found existing report:", cpg_tsv_path))
    log_msg("[CPG REPORT] Skipping generation (use --force_cpg to regenerate)")
  } else {
    if (isTRUE(opt$force_cpg) && file.exists(cpg_tsv_path)) {
      log_msg("[CPG REPORT] --force_cpg set, regenerating report...")
    }

    # Collect all DMR summaries from the analysis
    all_dmr_files <- list.files(opt$output_dir,
      pattern = "dmr_summary_.*\\.csv$",
      full.names = TRUE, recursive = TRUE
    )
    ...
    if (length(all_dmr_files) > 0) {
      # Read and combine DMR summaries
      all_dmrs <- data.table::rbindlist(lapply(all_dmr_files, function(f) {
        df <- read.csv(f, stringsAsFactors = FALSE)
        df$source_file <- basename(f)
        df
      ...
      # Rename columns to match expected format (scaffold -> chr, etc.)
      if ("scaffold" %in% names(all_dmrs) && !"chr" %in% names(all_dmrs)) {
        all_dmrs$chr <- all_dmrs$scaffold
      }
      ...
      if (all(c("chr", "start", "end") %in% names(all_dmrs))) {
        ...
        tryCatch(
          {
            ...
            # Generate Genome-Wide Manhattan Plot on Rerun
            # Look for ALL_DMRs file in both main dir and complete_dmr_datasets subdir
            dmr_files <- list.files(opt$output_dir, pattern = "ALL_DMRs_complete_dataset_.*\\.csv", full.names = TRUE, recursive = TRUE)
            if (length(dmr_files) > 0) {
              log_msg(paste("[Plot] Found DMR file for Manhattan plot:", basename(dmr_files[1])))
              generate_dmr_manhattan(dmr_files[1], opt$output_dir)
        ...
        log_msg("[CPG REPORT] DMR files missing required columns (chr, start, end)")
      }
    } else {
      log_msg("[CPG REPORT] No DMR summary files found - skipping CpG report")
    }
  } # End else for cache check
}
...
log_msg("[VALIDATION] Checking for key output files...")
for (file_name in key_files) {
  file_path <- file.path(opt$output_dir, file_name)
  if (file.exists(file_path)) {
    file_size <- file.info(file_path)$size
    log_msg(sprintf("  ✓ %s (%s bytes)", file_name, format(file_size, big.mark = ",")))
  } else {
    log_msg(sprintf("  ✗ %s (MISSING)", file_name))
  }
}
...
log_msg("")
log_msg("[VALIDATION] Checking subdirectories...")
for (subdir_name in subdirs) {
  subdir_path <- file.path(opt$output_dir, subdir_name)
  if (dir.exists(subdir_path)) {
    n_files <- length(list.files(subdir_path, recursive = TRUE))
    log_msg(sprintf("  ✓ %s/ (%d files)", subdir_name, n_files))
  } else {
    log_msg(sprintf("  ✗ %s/ (MISSING)", subdir_name))
  }
}
...
cat(paste0("[COMPLETE] Analysis complete! | MethylSense v", SCRIPT_VERSION, "\n"))
